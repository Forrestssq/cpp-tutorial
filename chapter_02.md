...
为了演示CMake的多文件项目的编译方法，我们先来构建一个词法分析器（lexer）。
...
静态库（Static Library）在链接时被直接嵌入到可执行文件里。静态库就是把一堆 `.cpp` 编译好的机器码打包成一个文件（.a），方便别人链接用，不用每次都重新编译源码。
...
ar 是一个打包工具（archive），这条命令的意思是：

```
ar  rcs  liblexer.a  lexer.o
↑    ↑       ↑          ↑
工具 选项  要创建的库  要打包进去的文件
```

rcs 是三个选项合在一起：

r — 把文件插入（replace）到库里，如果已存在就替换
c — 创建（create）库文件，如果不存在就新建
s — 建立索引（symbol index），让链接器能快速找到函数

所以整句话就是：把 lexer.o 打包成一个叫 liblexer.a 的静态库文件。
...
会发现 `main.i` 比 `main.cpp` 大很多——因为 `<iostream>` 被展开进去了，展开后能有几万行。
...
它等价于传统的 include guard（C和C++中都能用）：

```cpp
#ifndef LEXER_H   // 如果 LEXER_H 这个宏没有被定义过
#define LEXER_H   // 就定义它
// 头文件内容...
#endif            // 结束
```
...
> 注意这里面重新弄了一main，之前的lexer可以删了，新建一个`main.cpp`。
...
```bash
jln -s build/compile_commands.json compile_commands.json
cmake -B build
cmake --build build
./build/taco
```
...
ln 这行是在项目根目录创建一个指向 build/compile_commands.json 的软链接。clangd 默认在根目录寻找这个文件，读取后就能获得 ftxui 的头文件路径，从而提供代码补全和跳转功能。

为什么要先`cmake -B build`再`cmake --build build`？因为第一步是构建`MakeFile`文件，第二步是让g++去编译。
